(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))o(e);new MutationObserver(e=>{for(const s of e)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function t(e){const s={};return e.integrity&&(s.integrity=e.integrity),e.referrerpolicy&&(s.referrerPolicy=e.referrerpolicy),e.crossorigin==="use-credentials"?s.credentials="include":e.crossorigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(e){if(e.ep)return;e.ep=!0;const s=t(e);fetch(e.href,s)}})();const i=document.querySelector("#suggested-channels-content"),c=async()=>{const n=["https://youtube-v31.p.rapidapi.com/channels?part=snippet%2Cstatistics&id=UC8LeXCWOalN8SxlrPcG-PaQ","https://youtube-v31.p.rapidapi.com/channels?part=snippet%2Cstatistics&id=UCHRp19HU7Y2LwfI0Ai6WAGQ","https://youtube-v31.p.rapidapi.com/channels?part=snippet%2Cstatistics&id=UCJZv4d5rbIKd4QHMPkcABCw","https://youtube-v31.p.rapidapi.com/channels?part=snippet%2Cstatistics&id=UCw05fUBPwmpu-ehXFMqfdMw"],t={method:"GET",headers:{"X-RapidAPI-Key":"85888d94acmshd5891289e10b3bap1fa039jsn62ae81abbc20","X-RapidAPI-Host":"youtube-v31.p.rapidapi.com"}};try{return await Promise.all(n.map(e=>fetch(e,t).then(s=>s.json())))}catch(o){console.log("Error",o)}};(async()=>{try{const n=await c();let a="";n.map(t=>{t.items.map(o=>{a+=`<section class="gap-4 items-center shadow-lg w-full xl:gap-4 md:grid md:grid-cols-2 rounded-lg">
        ...
        `})}),i.innerHTML=a}catch(n){console.log(n)}})();const l="https://youtube-v31.p.rapidapi.com/search?channelId=UCgVjdxflkNOnn6FWPONXpIQ&part=snippet%2Cid&order=date&maxResults=10",p=document.getElementById("english-academy"),d="85888d94acmshd5891289e10b3bap1fa039jsn62ae81abbc20",u={method:"GET",headers:{"X-RapidAPI-Key":d,"X-RapidAPI-Host":"youtube-v31.p.rapidapi.com"}};async function h(n){return await(await fetch(n,u)).json()}(async()=>{try{let a=`${(await h(l)).items.map(t=>`
        ...
        `).slice(0,4).join("")}`;p.innerHTML=a}catch(n){console.log(n)}})();
